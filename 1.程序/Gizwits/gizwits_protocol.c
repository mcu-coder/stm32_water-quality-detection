...
static int8_t ICACHE_FLASH_ATTR gizCheckReport(dataPoint_t *cur, dataPoint_t *last)
{
    ...
    if(last->valuezhuodu != cur->valuezhuodu)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valuezhuodu Changed\n");
            gizwitsProtocol.waitReportDatapointFlag.flagzhuodu = 1;
            ret = 1;
        }
    }
    if(last->valueshuiwen != cur->valueshuiwen)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valueshuiwen Changed\n");
            gizwitsProtocol.waitReportDatapointFlag.flagshuiwen = 1;
            ret = 1;
        }
    }
    if(last->valueTDS != cur->valueTDS)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valueTDS Changed\n");
            gizwitsProtocol.waitReportDatapointFlag.flagTDS = 1;
            ret = 1;
        }
    }
...
static int8_t ICACHE_FLASH_ATTR gizDataPoints2ReportData(dataPoint_t *dataPoints , uint8_t *outData,uint32_t *outDataLen)
{
    ...
    if(gizwitsProtocol.waitReportDatapointFlag.flagzhuodu)
    {
        devStatusTmp.valuezhuodu = exchangeBytes(gizY2X(zhuodu_RATIO,  zhuodu_ADDITION, dataPoints->valuezhuodu)); 
        gizMemcpy((uint8_t *)&allDatapointByteBuf[byteFieldOffset],(uint8_t *)&devStatusTmp.valuezhuodu,zhuodu_LEN);
        byteFieldOffset += zhuodu_LEN;
    }
    if(gizwitsProtocol.waitReportDatapointFlag.flagshuiwen)
    {
        devStatusTmp.valueshuiwen = exchangeBytes(gizY2XFloat(shuiwen_RATIO,  shuiwen_ADDITION, dataPoints->valueshuiwen));     
        gizMemcpy((uint8_t *)&allDatapointByteBuf[byteFieldOffset],(uint8_t *)&devStatusTmp.valueshuiwen,shuiwen_LEN);
        byteFieldOffset += shuiwen_LEN;
    }
    if(gizwitsProtocol.waitReportDatapointFlag.flagTDS)
    {
        devStatusTmp.valueTDS = exchangeBytes(gizY2X(TDS_RATIO,  TDS_ADDITION, dataPoints->valueTDS)); 
        gizMemcpy((uint8_t *)&allDatapointByteBuf[byteFieldOffset],(uint8_t *)&devStatusTmp.valueTDS,TDS_LEN);
        byteFieldOffset += TDS_LEN;
    }
